...
- **Relazioni tipizzate**: dichiara relazioni strutturate (`crew`, `member_of`, `located_in`, `hostile_to`, `owns`, `owes_debt_to`, `reports_to`, `allied_with`, `mentor_of`) nel frontmatter YAML di un file. Query dirette e inverse gratuite (dichiari da un lato solo, l'altro compare da solo), indice live per root d'archivio, endpoint `/api/entities/{key}/relations` e `/api/diagnostics/relations`, sezione RELAZIONI nell'editor. Vedi `docs/guida-relazioni-tipizzate.md`.
